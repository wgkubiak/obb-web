...
const EditExamForm = (props) => {
    ...
    const [medicineType, setMedicineType] = useState(props.data.medicineType || "");
    ...
    const [respiratorySys, setRespiratorySys] = useState(props.data.respiratorySys || "");
  ...
  const data = useMemo(
    ...
    [
      exDate, exTime, feces, tissue, exResult, medicine, medicineQty, medicineType, diarrhea, pigWeight, temperature, lameness, respiratorySys, skinChanges
    ]
  );
  ...
  return (
    <div style={{position: "relative", display: "flex", flexDirection: "row", flexWrap: "wrap"}}>
      ...
      <StyledConfirmButton variant="success" onClick={event => submitHandler(event)}>POTWIERDŹ</StyledConfirmButton>
    </div>
  );
};
